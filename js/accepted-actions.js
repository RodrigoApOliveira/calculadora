...
function handlePercentage(state) {
  ...
  const expressionEndsWithSumOrSubtract =
    state.expression.match(lastNumberAndSignal)
...
function handleNegate(state) {
  const isNoEmptyAndZero =
    state.currentValue.length !== 0 && state.currentValue !== "0"
...
function handleSignalClick(state, signal) {
  ...
  const canAddSignal =
    ...
    endsWithNumberOrDot.test(state.expression)
